...
def get_flex_agg(login_url, user, pw, years_to_get, col_codes):
    ...
    login_url = "https://www.trueup.io/layoffs"
    browser.get(login_url)
    

    # Use a loop to click the "Show more" button until it no longer appears
    while True:
        ...
        # Optional: wait a moment for the page to load more items after clicking
        # Adjust the sleep time as necessary based on network speed and response time
        time.sleep(1)

    
    # find a "Show more button" on page
    while True:
        try:
            show_more_button = browser.find_element(By.CSS_SELECTOR, "[data-test='show-more-button']")
