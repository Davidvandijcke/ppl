...
prettyPlot <- function(disco, discot, t_list, ylim=c(-100,50), ylab="Change in Tenure (Days)",
                       xlab = "Quantile", lw=1, ncol=2) {
  # plot nicer figure
  ...
  p <- ggplot(df, aes(x = unlist(df$x))) +
    geom_line(aes(y = unlist(df$y)), colour = db, linetype = lty, linewidth=lw)

  if (!is.null(obsLine)) p <- p + geom_line(aes(y = obsLine), colour =db,
                                            linetype = lty_obs, show.legend = !is.null(obsLine))
  ...
  if (CI) p <- p + geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper), fill = lb, alpha = .5)
  ...
  clr <- "darkgrey"
  p <- p + facet_wrap(~time, ncol = ncol, nrow = n_row) + theme(strip.text = ggtext::element_markdown(size=8),
...
prettyHist <- function(disco, discot, t_list, ylim=c(-100,50), ylab="Change in Tenure (Days)",
  ...
  p <- p +
    geom_histogram(stat="identity", fill = lb, alpha=0.7) +
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = .2, color = db, alpha=0.7, linewidth=0.6) +
...
printWeights <- function(disco) {
  # print weights
  df <- data.table("weights" = as.vector(disco$weights), "id_col" = disco$control_ids)
...
robustness <- function(rtocomp) {
  ...
  grouped <- grouped[, .(y_col = max(y_col), month=min(month)),
                     by=c("time_col", "ID", "id_col")]  # %>% group_by(time_col, ID, id_col) %>% summarize(y_col = max(y_col), month=min(month))
  ...
  ## full distribution
  disco <- DiSCos::DiSCo(grouped, id_col.target = id_col.target, t0 = t0, q_min=0, q_max=0.9, G=10, M=M, num.cores = 20,
  ...
  p
  p <- p + scale_x_continuous(breaks=sort(unique(grouped$y_col)),
                              labels=list("0" = "0-5", "5"="5-10",
                                          "10" = "10+"))
  p<- p + theme(axis.text.x = element_text(angle = 90, hjust=1))
  ggsave(file.path(figs, paste0("tenure_years_", rtocomp, ".pdf")), p,  width = 4, height = 6,  dpi = 300)
